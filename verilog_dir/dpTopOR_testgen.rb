...
def gen_test1( d0, d1 )
# case of "exp0 |-> (  ( exp1 ##[1:%d] exp2 ) or ( exp1 ##[1:%d] exp3 ) ) ##1 exp4"

  test_init
  #single pass case on exp2
  1.upto(d0) do |t0| 
    set_trigger( 0, "exp0" ) 
    set_trigger( $nov, "exp1" ) 
    set_trigger( $nov+t0, "exp2" ) 
    set_trigger( $nov+t0+1, "exp4" ) 
    set_check( $nov+t0+3, "err || !mtch || ovwp" ) 
    $t_count += $nov+t0+4 ;
   end
  ...
  1.upto(d0) do |t0| 
    ...
    set_trigger( $nov+t0, "exp2" ) 
    #set_trigger( $nov+t0+1, "exp4" ) 
    set_expect_error( $nov+t3+2, 2, "err && !mtch && !ovwp" ) 
    $t_count += $nov+t3+4 ;
   end
  ...
  set_trigger( $nov, "exp1", 2 ) 
  set_trigger( $nov+1, "exp2",1 ) 
  set_trigger( $nov+2, "exp3",1 ) 
  set_trigger( $nov+2, "exp4",2 ) 
  set_check( $nov+4, "err || !mtch || ovwp" ) 
  set_check( $nov+5, "err || !mtch || ovwp" ) 

  $t_count += $nov+5 ;
 
  #double, first one pass case
  if( d0 > d1 )
    t3 = d0 
  else
    t3 = d1 
  ...
  set_trigger( $nov, "exp1", 2 ) 
  set_trigger( $nov+1, "exp2",1 ) 
  #set_trigger( $nov+1, "exp3",2 ) 
  set_trigger( $nov+2, "exp4",2 ) 
  set_check( $nov+4, "err || !mtch || ovwp" ) 
  set_expect_error( $nov+t3+3, 2, "err && !mtch && !ovwp" ) 
  ...
  set_trigger( 0, "exp0", 2 ) 
  set_trigger( $nov, "exp1", 2 ) 
  set_trigger( $nov+1, "exp3",1 ) 
  #set_trigger( $nov+1, "exp2",2 ) 
  set_trigger( $nov+2, "exp4",2 ) 
  set_check( $nov+4, "err || !mtch || ovwp" ) 
  set_expect_error( $nov+t3+3, 2, "err && !mtch && !ovwp" ) 
...
$test = sprintf( "exp0 |-> (  ( exp1 ##[1:%d] exp2 ) or ( exp1 ##[1:%d] exp3 ) ) ##1 exp4", d0, d1 )
$nov = 0 
gen_test1( d0, d1 )
fname = sprintf(  "dp_exp_2_p_seq%d_OR_seq%d_p_D_exp", d0, d1 )
gen_test_file( fname )
$test = sprintf( "exp0 |=> (  ( exp1 ##[1:%d] exp2 ) or ( exp1 ##[1:%d] exp3 ) ) ##1 exp4", d0, d1 )
$nov = 1
gen_test1( d0, d1 )
fname = sprintf(  "dp_exp_3_p_seq%d_OR_seq%d_p_D_exp", d0, d1 )
gen_test_file( fname )
