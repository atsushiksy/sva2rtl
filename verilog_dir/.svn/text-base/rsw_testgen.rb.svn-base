...
# case of exp0 ##1 (exp1 ##1 exp2) [*d0:d1] ##1 exp3 |->
#                           ( exp4 ##1 exp5 )[*d2:d3] ##1 exp6
  test_init
  #pass case
  d0.upto(d1) { |t0|
    1.upto(t0-d0+1) { |t1|
      d2.upto(d3) { |t2|
        1.upto(t2-d2+1) { |t3|
          ...
          1.upto(t0) {
            set_trigger( t, "exp1" ) 
            set_trigger( t+1, "exp2" ) 
            t += 2 
          }
          set_trigger( ((d0-1) * 2) + (t1 * 2)+1, "exp3" ) 
          $t_count += ((d0-1) * 2) + (t1 * 2)+1 + $nov 
          t = 0 
          1.upto(t2) {
            set_trigger( t, "exp4" )
            set_trigger( t+1, "exp5" )
            t += 2 
          }
          set_trigger( ((d2-1) * 2) + (t3 * 2), "exp6" ) 
          $t_count += ((d2-1) * 2) + (t3 * 2)
    ...
    1.upto(t0-d0+1) { |t1|
      d2.upto(d3) { |t2|
        1.upto(t2-d2+1) { |t3|
          ...
          $t_count += (t2 * 2) 
          #set_trigger( ((d2-1) * 2) + (t3 * 2), "exp6" ) 
          set_expect_error( 1, 5 , "err && !mtch && !ovwp" )
    ...
    1.upto(t0-d0+1) { |t1|
      d2.upto(d3) { |t2|
        1.upto(t2-d2+1) { |t3|
          ...
          set_trigger( tx+((d0-1) * 2) + (t1 * 2)+1, "exp3" ) 
          set_expect_overwrap( tx+((d0-1) * 2) + (t1 * 2)+1, 4 , "!err && ovwp" )
...
# case of exp0 ##1 (exp1 ##1 exp2) [*d0:d1] ##1 exp3 |-> ( exp4 ##1 exp5 )[*d2:d3] ##1 exp6
set_verilog_exp(7)
1.upto(5) do  |d1|
1.upto(d1) do  |d0|
1.upto(5) do  |d3|
...
$test = sprintf( "exp0 ##1 (exp1 ##1 exp2) [*%d:%d] ##1 exp3 |-> ( exp4 ##1 exp5 )[*%d:%d] ##1 exp6", d0, d1, d2, d3 )
$nov = 0 
gen_test1( d0, d1, d2, d3 )
fname = sprintf(  "exp_1_seqRW_1_exp_2_seqRW_1_exp_%d_%d_%d_%d", d0, d1, d2, d3 )
gen_test_file( fname )
$test = sprintf( "exp0 ##1 (exp1 ##1 exp2) [*%d:%d] ##1 exp3 |=> ( exp4 ##1 exp5 )[*%d:%d] ##1 exp6", d0, d1, d2, d3 )
$nov = 1
gen_test1( d0, d1, d2, d3 )
fname = sprintf(  "exp_1_seqRW_1_exp_3_seqRW_1_exp_%d_%d_%d_%d", d0, d1, d2, d3 )
gen_test_file( fname )
