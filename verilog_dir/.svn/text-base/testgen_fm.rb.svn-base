...
testfile = ARGV[0] ;
outfilepref = testfile.split( /\./ )[0] + "_fm"
...
if ($delay == 0) && ($range == 0 )
  ...
  set_seq( t_count,                "      exp0 = 1 ;\n" )
  set_seq( t_count+1,              "      exp0 = 0 ;\n" )
  set_exp( t_count+3,   "      if( !err || mtch || ovwp ) $display( \"%0d Error! unexpected responce\", cycle_count ) ;\n" )
  t_count += $nov+4 
...
if ($delay == 2) && ($range == 0 )
  ...
  1.upto(5) do | d0 |
    ...
    1.upto(5) do | d1 |
      ...
      set_exp( t_count+d0+$nov+d1+2,   "      if( err || !mtch ) $display( \"%0d Error! unexpected responce\", cycle_count ) ;\n" )
      t_count += d0+$nov+d1+3
      final_count_init += "  final_count = #{t_count} ;\n"
  ...
  1.upto(d2) do | d1 |
    ...
    if( d1 > 1 )
      1.upto(d1-1) do | t0 |
        ...
        set_seq( t_count+d0+$nov+t0,     "      exp3 = 1 ;\n" )
        set_seq( t_count+d0+$nov+t0+1,   "      exp3 = 0 ;\n" )      
        set_exp( t_count+d0+$nov+d2+3,   "      if( !err || mtch || ovwp ) $display( \"%0d Error! unexpected responce\", cycle_count ) ;\n" )
        t_count += d0+$nov + d2 + 3
      end
    end
...
if ($delay == 1) && ($range == 1 ) && ( $p_delay == 0 ) && ( $p_range = 1 )
  ...
  $delay_0 = sprintf( "##%d", d2 )
    ...
    d0.upto(d1) do |t0|
      ...
      set_exp( t_count+t0+$nov+d2+3,   "      if( !err || mtch || ovwp ) $display( \"%0d Error! unexpected responce\", cycle_count ) ;\n" )
      t_count += t0+$nov + d2 + 4      
    end
    # overwrap cases
    if d2 > ( d0 + 1 ) 
      ...
      set_seq( t_count+d0+$nov+1+d0,      "      exp2 = 1 ;\n" )  
      ...
      set_exp( t_count+d0+$nov+1+d0+2,   "      if( err || !ovwp ) $display( \"%0d Error! unexpected responce\", cycle_count ) ;\n" )
      set_exp( t_count+d0+$nov+d2+2,   "      if( err ||! mtch ) $display( \"%0d Error! unexpected responce\", cycle_count ) ;\n" )
      t_count += d0 + d2 + 4            
    end
