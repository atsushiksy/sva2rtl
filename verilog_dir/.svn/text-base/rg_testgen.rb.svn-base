...
def gen_test1( d0, d1 )
# case of exp0 ##1 exp1[->d0] |-> exp2[->d1] ##1 exp3
  test_init
  ...
  1.upto(5) { |t0|
    1.upto(5) { |t1|
      0.upto(5) { |t2|
        ...
        $t_count += 1 + t2 + ((d0-1) * t0) + $nov
        if t1 == 1
          set_trigger( t2, "exp2", d1  )        
        else
          t = t2
          1.upto(d1) {
            set_trigger( t, "exp2" )
...
# case of exp0 ##1 exp1[->d1] |-> exp2[->d2] ##1 exp3
set_verilog_exp(4)
1.upto(5) do |d0|
1.upto(5) do  |d1|

$test = sprintf( "exp0 ##1 exp1[->%d] |-> exp2[->%d] ##1 exp3 ", d0, d1 )
$nov = 0 
gen_test1( d0, d1 )
fname = sprintf(  "exp_1_expRG_2_expRG_1_exp_%d_%d", d0, d1 )
gen_test_file( fname )
$test = sprintf( "exp0 ##1 exp1[->%d] |=> exp2[->%d] ##1 exp3 ", d0, d1 )
$nov = 1
gen_test1( d0, d1 )
fname = sprintf(  "exp_1_expRG_3_expRG_1_exp_%d_%d", d0, d1 )
gen_test_file( fname )
