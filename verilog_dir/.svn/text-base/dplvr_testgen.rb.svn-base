...
def gen_test1( d0, d1 )
# case of exp0 ##d0 (exp1, lvar0 = exp2) |-> exp3 ##d1 ( exp4 == lvar0 )
  test_init
  ...
  set_check( d0+d1+$nov+2 , "err || !mtch || ovwp" )
  $t_count += $nov+ d0+d1+5 
 ...
 #pass with exp2 == 0 case
  ...
  set_trigger( d0, "exp2"  )        
  set_trigger( d0+$nov, "exp3"  )        
  set_check( d0+d1+$nov+3 , "!err || mtch || ovwp" )
  $t_count += $nov+ d0+d1+5 
  ...
  #double pass with first exp2 case
  set_trigger( 0, "exp0", 2 ) 
  set_trigger( d0, "exp1", 2  )        
  set_trigger( d0, "exp2"  )        
  set_trigger( d0+$nov, "exp3",2  )        
  set_trigger( d0+d1+$nov, "exp4" )
  set_check( d0+d1+$nov+2 , "err || !mtch || ovwp" )
  set_check( d0+d1+$nov+3 , "err || !mtch || ovwp" )
  $t_count += $nov+ d0+d1+5 

  #double pass with delay
  2.upto(d0) do |t0|
  set_trigger( 0, "exp0" ) 
  set_trigger( t0+0, "exp0" ) 
  set_trigger( d0, "exp1"  )        
  set_trigger( t0+d0, "exp1"  )        
  set_trigger( d0, "exp2"  )        
  set_trigger( d0+$nov, "exp3"  )        
  set_trigger( t0+d0+$nov, "exp3"  )        
  set_trigger( d0+d1+$nov, "exp4" )
  set_check( d0+d1+$nov+2 , "err || !mtch || ovwp" )
  set_check( t0+d0+d1+$nov+2 , "err || !mtch || ovwp" )
  $t_count += t0+$nov+ d0+d1+5 
  end

  #tripple , one overflow
  set_trigger( 0, "exp0", 3 ) 
  set_trigger( d0, "exp1", 3  )        
  set_trigger( d0, "exp2"  )        
...
$test =  "exp0 ##d0 (exp1, lvar0 = exp2) |-> exp3 ##d1 ( exp4 == lvar0 ) "
$nov = 0 
gen_test1( d0, d1 )
fname = sprintf(  "dplvr_exp_D_expS_2_exp_D_expR_%d_%d", d0, d1 )
gen_test_file( fname )
$test =  "exp0 ##d0 (exp1, lvar0 = exp2) |=> exp3 ##d1 ( exp4 == lvar0 ) "
$nov = 1
gen_test1( d0, d1 )
fname = sprintf(  "dplvr_exp_D_expS_3_exp_D_expR_%d_%d", d0, d1 )
gen_test_file( fname )
