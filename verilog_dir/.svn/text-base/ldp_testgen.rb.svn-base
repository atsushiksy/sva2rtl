...
def gen_test1( d0, d1 )
# case of exp0 ##d0 exp1 |-> ##d1 exp2

  test_init
  #single pass case
  set_trigger( 0, "exp0" ) 
  set_trigger( d0, "exp1" ) 
  set_trigger( $nov+d0+d1, "exp2" ) 
  set_check( $nov+d0+d1+2, "err || !mtch || ovwp" ) 
  $t_count += $nov+d0+d1+3 ;

  #double pass case
  set_trigger( 0, "exp0", 2 ) 
  set_trigger( d0, "exp1", 2 ) 
  set_trigger( $nov+d0+d1, "exp2", 2 ) 
  set_check( $nov+d0+d1+2, "err || !mtch || ovwp" ) 
  set_check( $nov+d0+d1+3, "err || !mtch || ovwp" )
...
$test = sprintf( "exp0 ##%d exp1 |-> ##%d exp2", d0, d1 )
$nov = 0 
gen_test1( d0, d1 )
fname = sprintf(  "dp_exp_D_exp_2_D_exp_%d_%d", d0, d1 )
gen_test_file( fname )
$test = sprintf( "exp0 ##%d exp1 |=> ##%d exp2", d0, d1 )
$nov = 1
gen_test1( d0, d1 )
fname = sprintf(  "dp_exp_D_exp_3_D_exp_%d_%d", d0, d1 )
gen_test_file( fname )
