...
def gen_expD0expRep1thenexpRep2expRep3exp( d0, d1, d2, d3 )
  # case of exp0 ##d0 exp1[*d1] |-> exp2 [*d2] ##1 exp3 [*d3] ##1 exp4
  test_init

  #pass case
  set_trigger( 0, "exp0" ) 
  set_trigger( d0, "exp1", d1+1 )
  set_trigger( d0+d1+$nov, "exp2", d2+1 )
  set_trigger( d0+d1+$nov+d2+2, "exp3", d3+1 )
  set_trigger( d0+d1+$nov+d2+2+d3+1, "exp4" )
  set_check( d0+d1+$nov+d2+2+d3+2, "err || !mtch || ovwp" )
  
  $t_count += d0+d1+d2+d3+$nov+4
...
$test = sprintf( "exp0 ##%d exp1[*%d] |-> exp2 [*%d] ##1 exp3 [*%d] ##1 exp4 ", d0, d1, d2, d3 )
$nov = 0 
gen_expD0expRep1thenexpRep2expRep3exp( d0, d1, d2, d3 )
fname = sprintf(  "expDexpR_2_expRexpRexp_%d_%d_%d_%d", d0, d1, d2, d3 )
gen_test_file( fname )
$test = sprintf( "exp0 ##%d exp1[*%d] |=> exp2 [*%d] ##1 exp3 [*%d] ##1 exp4 ", d0, d1, d2, d3 )
$nov = 1
gen_expD0expRep1thenexpRep2expRep3exp( d0, d1, d2, d3 )
fname = sprintf(  "expDexpR_3_expRexpRexp_%d_%d_%d_%d", d0, d1, d2, d3 )
gen_test_file( fname )
