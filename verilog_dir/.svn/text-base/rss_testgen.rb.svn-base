...
def gen_test1( d0, d1 )
  # case of (exp0 ##1 exp1) [*d0] |-> ( exp2 ##1 exp3 )[*d1]  
  test_init
  #pass case
  1.upto(d0) {
    $t_count += 2
  ...
  1.upto(d1) { |t0|
    ...
    set_expect_error( 1, 4 , "err && !mtch && !ovwp" )  
    $t_count += 5 
  }
  ...
  if ((d0 * 2) +1 ) < (d1 * 2) 
    ...
    1.upto(d0) {
      t += 2
      set_trigger( t, "exp0" ) 
      set_trigger( t + 1, "exp1" ) 
    }
    $t_count += $nov + 1
    set_expect_overwrap( d0 * 2, 5, "!err && ovwp" )
    1.upto(d1) {
      set_trigger( 0, "exp2" )
      set_trigger( 1, "exp3" )
      $t_count += 2 ;
...
$test = sprintf( "(exp0 ##1 exp1) [*%d] |-> ( exp2 ##1 exp3 )[*%d]", d0, d1 )
$nov = 0 
gen_test1( d0, d1 )
fname = sprintf(  "seqR_2_seqR_%d_%d", d0, d1 )
gen_test_file( fname )
$test = sprintf( "(exp0 ##1 exp1) [*%d] |=> ( exp2 ##1 exp3 )[*%d]", d0, d1 )
$nov = 1
gen_test1( d0, d1 )
fname = sprintf(  "seqR_3_seqR_%d_%d", d0, d1 )
gen_test_file( fname )
