...
def gen_test1( d0, d1 )
# case of exp0 |-> exp1 ##[%d:%d] exp2 ##[%d:%d] exp3 
  ...
  d0.upto(d1) do |t0| 
    d0.upto(d1) do |t1|
      ...
      set_check( $nov+t0+t1+2, "err || !mtch || ovwp" ) 

      $t_count += $nov+t0+t1+5 ;
    end
  end
  
  # second exp2 pass case 
  d0.upto(d1-1) do |t0| 
      set_trigger( 0, "exp0" ) 
      set_trigger( $nov, "exp1" ) 
      set_trigger( $nov+t0, "exp2", 2 ) 
      set_trigger( $nov+t0+d1+1, "exp3" ) 
      set_check( $nov+t0+d1+3, "err || !mtch || ovwp" ) 
...
$test = sprintf( "exp0 |-> exp1 ##[%d:%d] exp2 ##[%d:%d] exp3 ", d0, d1, d0, d1 )
$nov = 0 
gen_test1( d0, d1 )
fname = sprintf(  "dp_exp_2_exp_B%d_%d_exp_B%d_%d_exp", d0, d1, d0, d1 )
gen_test_file( fname )
$test = sprintf( "exp0 |=> exp1 ##[%d:%d] exp2 ##[%d:%d] exp3 ", d0, d1, d0, d1 )
$nov = 1
gen_test1( d0, d1 )
fname = sprintf(  "dp_exp_3_exp_B%d_%d_exp_B%d_%d_exp", d0, d1, d0, d1 )
gen_test_file( fname )
